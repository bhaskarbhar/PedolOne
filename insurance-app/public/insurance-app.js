...
consentForm.addEventListener('submit', async (e) => {
    ...
    if (!aadhaar && !pan && !passport && !drivinglicense) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/insurance/consent`, {
            method: 'POST',
            headers: {
...
document.getElementById('otpForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/insurance/verify-otp`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
resendBtn.addEventListener('click', async () => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/insurance/resend-otp`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
document.getElementById('drivinglicense').addEventListener('input', function(e) {
    let value = e.target.value.replace(/[^A-Z0-9]/g, '').toUpperCase();
    e.target.value = value;
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add hover effects to insurance product cards
    const productCards = document.querySelectorAll('.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-4 > div');
